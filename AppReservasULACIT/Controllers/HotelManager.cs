...
namespace AppReservasULACIT.Controllers
{
    public class HotelManager
    {
        ...
        public async Task<Hotel> Ingresar(Hotel hotel, string token)
        {
            ...
            var response = await httpClient.PostAsync(Url, new StringContent(JsonConvert.SerializeObject(hotel), Encoding.UTF8, "application/json"));
        ...
        public async Task<Hotel> Actualizar(Hotel hotel, string token)
        {
            ...
            var response = await httpClient.PutAsync(Url, new StringContent(JsonConvert.SerializeObject(hotel), Encoding.UTF8, "application/json"));
            return JsonConvert.DeserializeObject<Hotel>(await response.Content.ReadAsStringAsync());
        }
